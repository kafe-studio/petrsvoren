...
import { HERO_DEFAULTS, type HeroSettings } from "../../../lib/content";
...
// Uloží nastavení pozadí hlavní stránky (hero) jako JSON do site_texts.
export const POST: APIRoute = async ({ request }) => {
  ...
  const settings: HeroSettings = {
    imageUrl:
      typeof body.imageUrl === "string" && body.imageUrl.trim()
        ? body.imageUrl.trim()
        : HERO_DEFAULTS.imageUrl,
    opacity: clamp(body.opacity, 0, 100, HERO_DEFAULTS.opacity),
    position:
  ...
  await db
    ...
    .values({ key: "hero_settings", value: JSON.stringify(settings) })
    .onConflictDoUpdate({
